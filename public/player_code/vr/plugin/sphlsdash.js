...
SaranyuHlsHTML5Player.defaultOptions = {
		'debug' : 'false', //flag to check if //console.log to print or not
...
SaranyuHlsHTML5Player.MediaPlayer.prototype = {
		...
		_createFullControls : function(){
			...
			t._createFullControls.hideFullControls = function(){
				...
				t.playerLayers.addClass("hide-cursor");

			};

			t._createFullControls.showFullControls = function(){
				t.fullControls.show();
				t.playerLayers.removeClass("hide-cursor");
		...
		_createAndAppendHLStoPlayer : function(file){
				...
				t._createAndAppendHLStoPlayer._attachDashtoVideo = function (){
						...
						t.mediaElement.videoElement.dashObj.initialize( t.mediaElement.videoElement, content_url, true);
		...
		_buildplaypause : function(){
			...
			if(SaranyuHlsHTML5Player.Utils.mobileAndTabletcheck()){
				try{
					SaranyuHlsHTML5Player.Utils.DLOG("Pause triggered");
					t.fullControls.bottomControlBar.bottomPlayerControls.saranyuPlaypause.addClass("sp-pause");
					t.fullControls.bottomControlBar.bottomPlayerControls.saranyuPlaypause.removeClass("sp-play");
			...
			t._buildsubtitles._buildSubtitlesPopup = function(trackinfo){
              ...
              console.log("Check playlist index for subtitles"+playlistIndex +t.options.file[playlistIndex].subtitles);
				...
				function fetchAndRenderSubtitles(choosenIndex){
					...
					$.ajax({
						url: t.options.file[playlistIndex].subtitles[choosenIndex].file,
						context : t,
				...
				if((t.options.relatedvideoautoplay=="true" || t.options.relatedvideoautoplay==true) && (t.options.relatedvideos.length > 0))
				{
					    ...
					    if(nextIndexToBePlayer <= t.options.relatedvideos.length){
							...
							$(t.fullControls.relatedPanel.find(".sp-related-panel-autotext").html(t.options.relatedvideos[nextIndexToBePlayer].videotitle));
								...
								t.fullControls.relatedPanel.relatedVideosDisplayTimerCount = 9;
		...
		*/
		...
		_build360 : function(){
		  SaranyuHlsHTML5Player.Utils.DLOG("building VR");

		  var t = this;

	      t._build360.panorama, t._build360.viewer;

	      t._build360.panorama = new PANOLENS.VideoPanorama( '', { autoplay: true, saranyuPlayer : t } );

	      t._build360.viewer = new PANOLENS.Viewer(t,{controlBar: false});
	      t._build360.viewer.add( t._build360.panorama );

		  t.playerLayers.addClass('vr-active');
		  $(t.mediaElement.videoElement).hide();


	      /* mode changes */
	      // hlsPlayer._build360.viewer.mode = PANOLENS.Modes.NORMAL
	      // hlsPlayer._build360.viewer.mode = PANOLENS.Modes.CARDBOARD
	      // hlsPlayer._build360.viewer.mode = PANOLENS.Modes.STEREO

	      // when normal bring video element to foreground , and enable big play icons, meanwhile display : none for panolens
	      // when cardboard or stero, bring panolens into foreground and disable big play icons, meanwhile display : none for video element

	      /* sensor or mouse */
	      // hlsPlayer._build360.viewer.enableControl(0) // mouse
	      // hlsPlayer._build360.viewer.enableControl(1) // sensor


		  if(SaranyuHlsHTML5Player.Utils.mobileAndTabletcheck()){
			//setTimeout(function(){
				try{
					t._build360.viewer.enableControl(1);
				}catch(e){
				}				
			//}.bind(t),1000)
		  }
			...
			SaranyuHlsHTML5Player.Utils.DLOG("Attaching 360, stero and cardboard");
			var t = this;
			var media = t.mediaElement.videoElement;
			var options = t.options;
			var buttonName = 'VR'
			var vr = '<div class="sp-vr-switch sp-vr-switch-wrap"><button class="sp-vr-switch-btn"></button><div class="sp-vr-switch-wrap sp-vr-switch-popup-wrap"></div></div>';
			t.fullControls.bottomControlBar.bottomPlayerControls.append(vr);
			t.fullControls.bottomControlBar.bottomPlayerControls.vr = t.fullControls.bottomControlBar.bottomPlayerControls.find('.sp-vr-switch-wrap');
			t.fullControls.bottomControlBar.bottomPlayerControls.vr.button = t.fullControls.bottomControlBar.bottomPlayerControls.vr.find('.sp-vr-switch-btn');
			t.fullControls.bottomControlBar.bottomPlayerControls.vr.popup = t.fullControls.bottomControlBar.bottomPlayerControls.vr.find('.sp-vr-switch-wrap');

			t.fullControls.bottomControlBar.bottomPlayerControls.vr.button.html(buttonName);

			t._buildmultiaudio._buildVRPopup = function(trackinfo){
				t.fullControls.bottomControlBar.bottomPlayerControls.vr.popup.hide();
				t.fullControls.bottomControlBar.bottomPlayerControls.vr.popup.empty();
				SaranyuHlsHTML5Player.Utils.DLOG("Attaching vr popup elements");

				function addEventsToVRSelectionButton(button){
					button.find('button').unbind().click(function(event){
						var index = Number($(event.target).attr("index"));
						var cmd = $(event.target).attr("cmd");
						var label = $(event.target).html();
						var media = t.mediaElement.videoElement;
						if(cmd == "normal"){
							try{
								t._build360.viewer.enableEffect(1);
							}catch(e){								
							}
							t._build360.viewer.onWindowResize();
						}else if(cmd == "cardboard"){
							try{
								t._build360.viewer.enableEffect(2);
							}catch(e){								
							}									
							t._build360.viewer.onWindowResize();					
						}else if(cmd == 'stereo'){	
							try{
								t._build360.viewer.enableEffect(3);
							}catch(e){								
							}
							t._build360.viewer.onWindowResize();
						}						
						...
						t.fullControls.bottomControlBar.bottomPlayerControls.vr.popup.hide();
						SaranyuHlsHTML5Player.Utils.DLOG("vr index changed to "+index+"x or label "+label);
					}.bind(t));
				}
					...
					//if(SaranyuHlsHTML5Player.Utils.mobileAndTabletcheck()){
						var vrinfo = [{'title':'Normal','cmd' : 'normal'},{'title':'Cardboard','cmd' : 'cardboard'},{'title':'Stereo','cmd' : 'stereo'}];
					// }else{
					// 	var vrinfo = [{'title':'Normal','cmd' : 'normal'},{'title':'Cardboard','cmd' : 'cardboard'}];
					// }


					$.each(vrinfo,function(index,value){
					SaranyuHlsHTML5Player.Utils.DLOG("Attaching vr switching with name of "+(value.title));

					var selectOptions = '';
					var classInactive = "inactive";
					var classActive = "active";
					var autoIndex = 0;
					var autoLabel = SaranyuHlsHTML5Player.defaultOptions.titleStrings.vrinfo;
					var label =  value.title;
					var cmd = value.cmd;

					selectOptions += "<div><button class="+classInactive+" type='button' cmd="+cmd+" label="+label+" index="+index+">"+label+"</button></div>" ;
					addEventsToVRSelectionButton(t.fullControls.bottomControlBar.bottomPlayerControls.vr.popup.append(selectOptions));
					});
				

			};
			t._buildmultiaudio._buildVRPopup();
			// hide and show the popup of vr switching
			t.fullControls.bottomControlBar.bottomPlayerControls.vr.button.unbind().click(function(event){
				t.fullControls.bottomControlBar.bottomPlayerControls.vr.popup.toggle();			
			}.bind(t));	      
...
(function () {
	...
	PANOLENS.Panorama.prototype.getZoomLevel = function () {
		...
		if ( window.innerWidth <= 800 ) {
			...
			zoomLevel = this.ImageQualityLow;Proceed
...
(function(){
	...
	PANOLENS.VideoPanorama.prototype.load = function ( src, options ) {
		...
		this.videoElement = options.saranyuPlayer.mediaElement.videoElement; 
...
(function () {
	...
	PANOLENS.Viewer = function ( saranyuPlayer , options) {		
		...
		this.options = options;
		this.saranyuPlayer = saranyuPlayer;
		...
		if ( options.container ) {
			...
			saranyuPlayer.mediaElement.prepend(container);
			this.containerPanolens = $(saranyuPlayer.mediaElement).find('.panolens-container')[0]


			// if(SaranyuHlsHTML5Player.Utils.mobileAndTabletcheck()){
			// 	$(document).click($(this.containerPanolens).find('canvas'),function(){					
			// 	  this.saranyuPlayer._createFullControls.showFullControls();					
			// 	}.bind(this))
			// }
...
*/
	...
	PANOLENS.Viewer.prototype.onWindowResize = function ( windowWidth, windowHeight ) {
		...
		width = this.saranyuPlayer.mediaElement.width();
		height = this.saranyuPlayer.mediaElement.height();
